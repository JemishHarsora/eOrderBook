                        ...
                        <div class="p-3">
                            <ul class="list-group list-group-flush">
                                @php
                                    ...
                                    $products_top_selling= filter_products(\App\Product::where('user_id', $detailedProduct->user_id)->orderBy('num_of_sale', 'desc'))->limit(6)->get();
                                    if($area_seller['seller_ids'] != null){
                                        if($area_seller['seller_ids']['0'] != null){
                                            $products_top_selling=filter_products(\App\Product::where('user_id', $detailedProduct->user_id)->whereIn('brand_id', $area_seller->brand_ids)->orderBy('num_of_sale', 'desc'))->limit(6)->get();
                                        }else{
                                            $products_top_selling=filter_products(\App\Product::where('user_id', $detailedProduct->user_id)->where('id', $area_seller['seller_ids']['0'])->orderBy('num_of_sale', 'desc'))->limit(6)->get();
                                        }
                                    }
 ...
 $retated_product= array();
                                    if($area_seller['seller_ids'] != null){
                                        if($area_seller['seller_ids']['0'] != null){
                                            ...
                                            $retated_product = filter_products(\App\Product::where('category_id', $detailedProduct->category_id)->where('id', $area_seller['seller_ids']['0'])->where('id', '!=', $detailedProduct->id))->limit(10)->get();
                                        }
                                    }
                                @endphp
