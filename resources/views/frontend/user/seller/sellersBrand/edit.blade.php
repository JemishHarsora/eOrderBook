        ...
        <div class="container">
            <div class="d-flex align-items-start">
                ...
                <div class="aiz-user-panel">
                    ...
                    <form class="" action="{{route('myBrands.update', $seller->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                                    ...
                                    <div class="col-md-8">
                                        <select class="form-control aiz-selectpicker" name="area_id" id="area_id" data-live-search="true" required>
                                            @foreach ($areas as $area)
                                                <option value="{{ $area->id }}" @if($seller->area_id == $area->id) selected @endif>{{ $area->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
