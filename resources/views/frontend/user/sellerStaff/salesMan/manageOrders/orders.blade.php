                            ...
                            <div class="card-header row gutters-5">
                                ...
                                <div class="col-md-3 ml-auto">
                                    <select class="form-control aiz-selectpicker"
                                        ...
                                        <option value="paid" @isset($payment_status) @if($payment_status=='paid' )
                                            selected @endif @endisset>{{ translate('Paid')}}</option>
                                        <option value="unpaid" @isset($payment_status) @if($payment_status=='unpaid' )
                                            selected @endif @endisset>{{ translate('Un-Paid')}}</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-3 ml-auto">
                                    <select class="form-control aiz-selectpicker"
                                        ...
                                        <option value="pending" @isset($delivery_status) @if($delivery_status=='pending'
                                            ) selected @endif @endisset>{{ translate('Pending')}}</option>
                                        <option value="confirmed" @isset($delivery_status)
                                            @if($delivery_status=='confirmed' ) selected @endif @endisset>
                                            {{ translate('Confirmed')}}</option>
                                        <option value="on_delivery" @isset($delivery_status)
                                            @if($delivery_status=='on_delivery' ) selected @endif @endisset>
                                            {{ translate('On delivery')}}</option>
                                        <option value="delivered" @isset($delivery_status)
                                            @if($delivery_status=='delivered' ) selected @endif @endisset>
                            ...
                            <table class="table aiz-table mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($order->orderDetails->where('seller_id',
                                            Auth::user()->created_by)->first()->payment_status == 'paid')
