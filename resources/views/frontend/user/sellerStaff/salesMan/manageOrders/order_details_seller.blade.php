...
@php
    $status = $order->orderDetails->where('seller_id', Auth::user()->created_by)->first()->delivery_status;
    $payment_status = $order->orderDetails->where('seller_id', Auth::user()->created_by)->first()->payment_status;
            ...
            <div class="card mt-4">
                ...
                <div class="card-body pb-0">
                    ...
                    <table class="table table-borderless table-responsive">
                        ...
                        <tbody>
                            @foreach ($order->orderDetails->where('seller_id', Auth::user()->created_by) as $key => $orderDetail)
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td>
                    ...
                    <table class="table table-borderless">
                        <tbody>
                            <tr>
                                ...
                                <td class="text-right">
                                    <span class="strong-600">{{ single_price($order->orderDetails->where('seller_id', Auth::user()->created_by)->sum('price')) }}</span>
                                </td>
                            </tr>
                            <tr>
                                <td class="w-50 fw-600">{{ translate('Shipping')}}</th>
                                <td class="text-right">
                                    <span class="text-italic">{{ single_price($order->orderDetails->where('seller_id', Auth::user()->created_by)->sum('shipping_cost')) }}</span>
                                </td>
                            </tr>
                            <tr>
                                <td class="w-50 fw-600">{{ translate('Tax')}}</th>
                                <td class="text-right">
                                    <span class="text-italic">{{ single_price($order->orderDetails->where('seller_id', Auth::user()->created_by)->sum('tax')) }}</span>
                                </td>
                            </tr>
                            <tr>
