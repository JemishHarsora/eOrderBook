    ...
    <div class="container-flude">
        <div class="px-2 py-4 px-md-4 py-md-3 bg-white shadow-sm rounded">
            ...
            @php
                ...
                $products= filter_products(\App\ProductPrice::with(['product'])->where('published', 1)->where('seller_featured', '1')->groupBy('product_id'))->limit(12)->get();
                if($area_seller['seller_ids'] != null){
                    if($area_seller['seller_ids']['0'] != null){
                        $products= filter_products(\App\ProductPrice::with(['product' => function($query) use($area_seller){
                            $query->whereIn('brand_id', $area_seller->brand_ids);
                        }])->where('published', 1)->whereIn('seller_id', $area_seller->seller_ids)->where('seller_featured', '1')->groupBy('product_id'))->limit(12)->get();
                    }else{
                        $products= filter_products(\App\ProductPrice::with(['product'])->where('published', 1)->where('seller_featured', '1')->where('seller_id', $area_seller['seller_ids'])->groupBy('product_id'))->limit(12)->get();
                    }
                }
            @endphp
