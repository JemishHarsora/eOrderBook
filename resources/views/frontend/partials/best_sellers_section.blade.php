@if (\App\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1)
    @php
        ...
        $sellers = \App\Seller::where('verification_status', 1)->get();
        if($area_seller['seller_ids'] != null){
            if($area_seller['seller_ids']['0'] != null){
            $sellers = \App\Seller::where('verification_status', 1)->whereIn('user_id', $area_seller->seller_ids)->get();
            }else{
                $sellers = \App\Seller::where('verification_status', 1)->where('user_id', $area_seller['seller_ids']['0'])->get();
            }
        }
